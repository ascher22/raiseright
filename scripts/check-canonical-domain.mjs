...
async function main() {
  ...
  for (const pattern of PLACEHOLDER_PATTERNS) {
    if (pattern.test(siteOrigin)) {
      if (process.env.PLACEHOLDER_OK === "1" || /REPLACE_ME/i.test(siteOrigin)) {
        console.warn(
          `Canonical domain warning: SITE_ORIGIN is a placeholder (${siteOrigin}); PLACEHOLDER_OK escape allows build.`,
        );
        continue;
      }
      console.error(
